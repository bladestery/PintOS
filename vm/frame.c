...
void frame_init(void)
{
    ...
    while ((kpage = palloc_get_multiple(PAL_USER, 1)) != NULL)
    {
        ...
        struct pool_elem *pool_elemt = (struct pool_elem *) malloc(sizeof(struct pool_elem));
        pool_elemt->frame = frame;
        
        lock_acquire(&frame_pool.lock);
        list_push_front(&frame_pool.pool, &pool_elemt->elem);
...
{
    struct frame *ret;
    struct pool_elem *temp;
    lock_acquire(&frame_pool.lock);
    temp = (struct pool_elem *) list_pop_front(&frame_pool.pool);
    lock_release(&frame_pool.lock);
    ret = temp->frame;
    ASSERT(ret);
    ASSERT(ret->phys_addr);
    /*free(temp);*/
    if (ret != NULL)
    {
        ret->vir_addr = ((struct page *) kpage)->vir_addr;
        ret->thread = thread_current();
        ret->page = (struct page *) kpage;
        ((struct page *) kpage)->phys_addr = ret->phys_addr;
        ((struct page *) kpage)->page_loc = in_memory;
        
        lock_acquire(&frame_lock);
        hash_insert(&frame_table, (struct hash_elem *) &ret->elem);
        lock_release(&frame_lock);
        
        struct evict_info *entry = (struct evict_info *) malloc(sizeof(struct evict_info));
        entry->phys_addr = ret->phys_addr;
    ...
    else
    {
        ...
        struct frame *f = NULL;
        lock_acquire(&evict.lock);
        for (e = list_begin(&evict.pool); e != list_end(&evict.pool); e = list_next(e))
        {
...
void free_frame(uint32_t *phys_addr)
{
    ...
    if (entry != NULL)
    {
        if (frame->page->page_loc == in_memory) {
            ...
            memset(frame->phys_addr + sizeof(uint32_t *), 0, PGSIZE - sizeof(uint32_t *));
            
            struct pool_elem* new = (struct pool_elem *) malloc(sizeof(struct pool_elem));
            new->frame = frame;
            
            lock_acquire(&frame_pool.lock);
            list_push_front(&frame_pool.pool, &new->elem);
            lock_release(&frame_pool.lock);
        }
...
{
    struct frame *frame = get_frame(((struct page *)page)->phys_addr);
    if (frame == NULL)
        return;
    else
        frame->is_pinned = pin;
...
void update_evict(uint32_t *phys_addr)
{
    ...
    for (e = list_begin(&evict.pool); e != list_end(&evict.pool); e = list_next(e))
    {
        ...
        {
            f = (struct evict_info *) list_remove(&f->elem);
            list_push_back(&evict.pool, &f->elem);
            break;
        }
    }
    lock_release(&evict.lock);
...
{
    while (!list_empty (&frame_pool.pool))
    {
        struct pool_elem *e = list_pop_front (&frame_pool.pool);
        free(e);
    }
}
